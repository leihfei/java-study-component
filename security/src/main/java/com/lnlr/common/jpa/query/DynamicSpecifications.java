    ...
    private static void setBuilder(CriteriaBuilder criteriaBuilder, List<Predicate> predicates, SearchFilter filter, Path expression, Class clazz) {
        ...
        switch (filter.getOperator()) {
            case EQ:
                ...
                break;
            case LIKE:
                predicates.add(criteriaBuilder.like(expression, PERCENTAGE_NUMBER + filter.getValue() + PERCENTAGE_NUMBER));
                break;
            case LLIKE:
                predicates.add(criteriaBuilder.like(expression, PERCENTAGE_NUMBER + filter.getValue()));
                break;
            case RLIKE:
                predicates.add(criteriaBuilder.like(expression, filter.getValue() + PERCENTAGE_NUMBER));
                break;
            case GT:
            ...
            case IN:
                predicates.add(criteriaBuilder.in(expression.in((Object[]) filter.getValue().toString().split(COMMA))));
                break;
            case ISNULL:
            ...
            case BETWEEN:
                ...
                if (LocalDateTime.class.isAssignableFrom(clazz)) {
                    predicates.add(criteriaBuilder.between(expression, getDate(filter.getValue(), 0), getDate(filter.getValue(), 1)));
                } else if (LocalDate.class.isAssignableFrom(clazz)) {
                    predicates.add(criteriaBuilder.between(expression, transform2LocalDate(((String) filter.getValue()).split(COMMA)[0])
                            , transform2LocalDate(((String) filter.getValue()).split(COMMA)[1])));
                } else if (String.class.isAssignableFrom(clazz)) {
                    if (filter.getValue() != null) {
                        predicates.add(criteriaBuilder.between(expression, ((String) filter.getValue()).split(COMMA)[0]
